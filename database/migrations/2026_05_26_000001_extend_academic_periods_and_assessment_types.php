...
return new class extends Migration
{
    public function up(): void
    {
        foreach (['assessments', 'student_averages', 'report_cards'] as $table) {
            if (Schema::hasTable("{$table}_legacy")) {
                ...
                if (! DB::table("{$table}_legacy")->exists()) {
                    Schema::drop("{$table}_legacy");
                }
            }
        }
    ...
    {
        if (Schema::hasTable('assessments_legacy') && ! Schema::hasTable('assessments')) {
            if (! DB::table('assessments_legacy')->exists()) {
        ...
        $hasLegacyData = DB::table('assessments')->exists();
        ...
        Schema::create('assessments', function (Blueprint $table) {
            $this->defineAssessmentsColumns($table);
        });
    ...
    private function createAssessmentsTable(): void
    {
        Schema::create('assessments', function (Blueprint $table) {
            $this->defineAssessmentsColumns($table);
        });
    ...
    private function defineAssessmentsColumns(Blueprint $table): void
    {
        $table->id();
    ...
    private function rebuildStudentAverages(): void
    {
        if (! Schema::hasTable('student_averages')) {
            Schema::create('student_averages', function (Blueprint $table) {
                $this->defineStudentAveragesColumns($table);
            });

            return;
        }

        $hasLegacyData = DB::table('student_averages')->exists();

        if ($hasLegacyData) {
            Schema::rename('student_averages', 'student_averages_legacy');
        } else {
            Schema::drop('student_averages');
        }
    ...
    private function defineStudentAveragesColumns(Blueprint $table): void
    {
        $table->id();
        $table->foreignId('student_id')->constrained('students')->onDelete('cascade');
        $table->foreignId('subject_id')->constrained('subjects')->onDelete('cascade');
    ...
    private function rebuildReportCards(): void
    {
        if (! Schema::hasTable('report_cards')) {
            Schema::create('report_cards', function (Blueprint $table) {
                $this->defineReportCardsColumns($table);
            });

            return;
        }

        $hasLegacyData = DB::table('report_cards')->exists();
    ...
    private function defineReportCardsColumns(Blueprint $table): void
    {
        $table->id();
        $table->foreignId('student_id')->constrained('students')->onDelete('cascade');
        $table->foreignId('class_id')->constrained('school_classes')->onDelete('cascade');
